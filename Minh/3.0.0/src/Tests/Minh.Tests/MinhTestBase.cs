...
using Minh.Migrations.SeedData;
...
namespace Minh.Tests
{
    public abstract class MinhTestBase : AbpIntegratedTestBase<MinhTestModule>
    {
        ...
        protected void UsingDbContext(Action<MinhDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MinhDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MinhDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MinhDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MinhDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MinhDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MinhDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MinhDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MinhDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MinhDbContext, Task<T>> func)
        {
            T result;
